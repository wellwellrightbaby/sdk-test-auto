...
export declare abstract class BaseCenter extends Common {
    ...
    sendCustomCommand(
        dstMembers: string[],
        customContent: string | Object,
        success: (seq: number, customContent: string) => void,
        error: (err: ERRO, seq: number, customContent: string) => void,
    ): boolean;
    ...
    sendBigRoomMessage(
        ...
        type: 1 | 2 | 3,
        content: string,
        success: (seq: number, messageId: string) => void,
        error: (err: ERRO, seq: number) => void,
    ): void;
